...
public static class OrchardNavigationPass
{
    ...
    public static void Apply(GameObject root, string assetPath)
    {
        ...
        Transform withdrawalButton = WithdrawalButtonSurface(root, assetPath);
        foreach (Image plate in images)
        {
            // Keep the withdrawal action clear when the prefab skin is reapplied.
            if (plate.transform == withdrawalButton) continue;
            ...
            if (role == "Title" || role == "ButtonGreen") AddLeafPair(plate, role == "Title");
        }
    }

    private static Transform WithdrawalButtonSurface(GameObject root, string assetPath)
    {
        switch (Path.GetFileNameWithoutExtension(assetPath))
        {
            case "FakeWithdrawPanel":
                return root.transform.Find("Content/WithdrawBtn/bg");
            case "RealWithdrawPanel":
                return root.transform.Find("Content/Scroll View/Viewport/Content/WithdrawInfo/WithdrawBtn/Btn");
            case "WithdrawInfo":
                return root.transform.Find("WithdrawBtn/Btn");
            default:
                return null;
        }
    }
