...
/// </summary>
[InitializeOnLoad]
public static class HudActionButtonStylePreview
{
    ...
    private const int CropPixelHeight = 240;
    private static readonly UTF8Encoding Utf8 = new UTF8Encoding(false);
    private static string DirectoryPath => Path.GetFullPath(Path.Combine(Application.dataPath, "../Design/HudActionButtonStyle-20260918"));
    ...
    {
        public string kind = "PREFAB STATIC DESIGN PREVIEW WITH EXPLICIT SAMPLE TEXT - NOT IMPLEMENTED UI OR GAMEPLAY";
        ...
        public bool longValues;
        public bool afterStyle;
        public string previewOnlyTexture;
    ...
    {
        public string kind = "PREFAB STATIC DESIGN PREVIEW BATCH - NOT IMPLEMENTED OR RUNTIME";
        public string generatedUtc;
        public string folder;
        public List<Report> cases = new List<Report>();
    }

    static HudActionButtonStylePreview()
    {
    ...
    private static void ReadCommand()
    {
        ...
        try
        {
            if (command != "preview" && !command.StartsWith("preview:", StringComparison.Ordinal))
                throw new ArgumentException("Allowed command: preview[:label] (before/after with ordinary/long sample values; preview clones only).");
            ...
            foreach (bool afterStyle in new[] { false, true })
                foreach (bool longValues in new[] { false, true })
                    queue.Enqueue(new Case
                    {
                        name = (afterStyle ? "after" : "before") + "-static-hud-1080x1920-" + (longValues ? "long" : "sample"),
                        height = 1920, longValues = longValues, currencyScale = 1f, afterStyle = afterStyle
                    });
            State.completed = 0;
    ...
    private static Report Render(Case item, string folder)
    {
        ...
        var report = new Report
        {
            ...
            currencyScale = item.currencyScale, longValues = item.longValues, afterStyle = item.afterStyle,
        ...
        try
        {
            ...
            if (item.afterStyle) ApplyButtonStyle(bar, report, out buttonTexture, out buttonSprite);
            ...
            camera.backgroundColor = new Color(.0f, .43f, .73f, 1f);
        ...
        finally
        {
            ...
            if (buttonSprite != null) Object.DestroyImmediate(buttonSprite);
            if (buttonTexture != null) Object.DestroyImmediate(buttonTexture);
    ...
    {
        SetText(bar, "Image/Text (TMP)", item.longValues ? "9999" : "2", report);
        SetText(bar, "CurrentGroup/GoldGroup/CoinBox/GoldText", item.longValues ? "99.999,99" : "0,08", report);
        SetText(bar, "CurrentGroup/DollarGroup/DollarBox/DollarText", item.longValues ? "R$9.999,99" : "210.36", report);
        ...
        // These are explicit Brazil currency samples on disposable clones; no localization or country service runs.
        SetSampleSprite(bar, "CurrentGroup/GoldGroup/CoinBox/Icon", "Assets/BizzaWZ/Final/Real/GameAssets/WzTexture_Money/GoldCoin_BR.png", report);
        SetSampleSprite(bar, "CurrentGroup/DollarGroup/DollarBox/Icon", "Assets/BizzaWZ/Final/Real/GameAssets/WzTexture_Money/StackMoney_BR.png", report);
    ...
    private static void ApplyButtonStyle(Transform bar, Report report, out Texture2D texture, out Sprite sprite)
    {
        string path = Path.Combine(DirectoryPath, "button-honey.png");
        if (!File.Exists(path)) throw new FileNotFoundException("Preview-only button image missing", path);
        texture = new Texture2D(2, 2, TextureFormat.RGBA32, false) { name = "PreviewOnlyHoneyButton", hideFlags = HideFlags.HideAndDontSave };
        if (!ImageConversion.LoadImage(texture, File.ReadAllBytes(path), false)) throw new InvalidDataException("Could not decode preview button.");
        if (texture.width != 1774 || texture.height != 887) throw new InvalidDataException("Preview image dimensions changed; recheck authored crop.");
        texture.filterMode = FilterMode.Bilinear;
        texture.wrapMode = TextureWrapMode.Clamp;
        // Bottom-left Unity rect: use verified opaque region, avoiding distant low-alpha noise.
        sprite = Sprite.Create(texture, new Rect(28f, 204f, 1718f, 486f), new Vector2(.5f, .5f), 100f, 0, SpriteMeshType.FullRect, new Vector4(160f, 60f, 160f, 60f));
        sprite.name = "PreviewOnlyHoneyButton";
        sprite.hideFlags = HideFlags.HideAndDontSave;
        TMP_Text body = Require(bar, "CurrentGroup/GoldGroup/CoinBox/GoldText").GetComponent<TMP_Text>();
        foreach (string buttonPath in new[] { "CurrentGroup/GoldGroup/ButtonView", "CurrentGroup/DollarGroup/ButtonView" })
        {
            Transform target = Require(bar, buttonPath);
            Image visual = target.GetComponent<Image>();
            if (visual == null) throw new InvalidDataException("Expected existing button Image at " + buttonPath);
            visual.sprite = sprite;
            visual.overrideSprite = sprite;
            visual.type = Image.Type.Sliced;
            visual.pixelsPerUnitMultiplier = 10f;
            foreach (TMP_Text text in target.GetComponentsInChildren<TMP_Text>(true))
            {
                text.fontSharedMaterial = body.fontSharedMaterial;
                text.color = body.color;
            }
        }
        report.previewOnlyTexture = path;
        report.sampleChanges.Add("DESIGN ONLY: two existing ButtonView Images use honey texture (9-slice); their existing TMP nodes use the amount-label navy material. Rects, hierarchy, Button components and event references remain unchanged.");
    }

    private static void SetSampleSprite(Transform bar, string path, string spritePath, Report report)
    {
        Image image = Require(bar, path).GetComponent<Image>();
        Sprite sprite = AssetDatabase.LoadAssetAtPath<Sprite>(spritePath);
        if (image == null || sprite == null) throw new InvalidDataException("Missing preview currency image: " + path);
        image.sprite = sprite;
        report.sampleChanges.Add(path + " sample sprite = " + spritePath);
    }
